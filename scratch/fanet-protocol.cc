...
{
    return 1 + 4 + 4 + 1 + 4 * neighbors.size(); // type + seq + nodeId + neighborCount + neighbors
}
...
FanetRoutingProtocol::UpdateRoutingTable()
{
    ...
    // 1. Seed routes with direct neighbors (HELLO)
    ...
    while (!q.empty())
    {
        ...
        {
            if (advertisedNbr == m_ipv4->GetAddress(1, 0).GetLocal())
            {
                continue;
            }
            ...
            entry.nextHop = firstHop[current];
            entry.interface = newTable[firstHop[current]].interface;
