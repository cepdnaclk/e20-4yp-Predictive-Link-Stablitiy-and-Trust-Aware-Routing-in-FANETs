...
#define QBR_MSG_HEADER_SIZE 12
#define QBR_PKT_HEADER_SIZE 4

namespace ns3
{

NS_LOG_COMPONENT_DEFINE("QbrHeader");
...
SecondsToEmf(double seconds)
{
    ...
    NS_ASSERT_MSG(seconds >= QBR_C, "SecondsToEmf - Can not convert a value less than QBR_C");

    // find the largest integer 'b' such that: T/C >= 2^b
    for (b = 1; (seconds / QBR_C) >= (1 << b); ++b)
    {
    }
    NS_ASSERT((seconds / QBR_C) < (1 << b));
    b--;
    NS_ASSERT((seconds / QBR_C) >= (1 << b));

    // compute the expression 16*(T/(C*(2^b))-1), which may not be a integer
    double tmp = 16 * (seconds / (QBR_C * (1 << b)) - 1);
...
///
/// @param qbrFormat number of seconds in mantissa/exponent format.
/// @return the decimal number of seconds.
///
double
EmfToSeconds(uint8_t qbrFormat)
{
    int a = (qbrFormat >> 4);
    int b = (qbrFormat & 0xf);
    // value = C*(1+a/16)*2^b [in seconds]
    return QBR_C * (1 + a / 16.0) * (1 << b);
}
...
{
    static TypeId tid = TypeId("ns3::qbr::PacketHeader")
                            .SetParent<Header>()
                            .SetGroupName("Qbr")
...
{
    static TypeId tid = TypeId("ns3::qbr::MessageHeader")
                            .SetParent<Header>()
                            .SetGroupName("Qbr")
...
{
    uint32_t size = QBR_MSG_HEADER_SIZE;
...
MessageHeader::Deserialize(Buffer::Iterator start)
{
    ...
    size = QBR_MSG_HEADER_SIZE;
    switch (m_messageType)
    {
    case MID_MESSAGE:
        size += m_message.mid.Deserialize(i, m_messageSize - QBR_MSG_HEADER_SIZE);
        break;
    case HELLO_MESSAGE:
        size += m_message.hello.Deserialize(i, m_messageSize - QBR_MSG_HEADER_SIZE);
        break;
    case TC_MESSAGE:
        size += m_message.tc.Deserialize(i, m_messageSize - QBR_MSG_HEADER_SIZE);
        break;
    case HNA_MESSAGE:
        size += m_message.hna.Deserialize(i, m_messageSize - QBR_MSG_HEADER_SIZE);
        break;
    default:
        NS_ASSERT(false);
    }
...
MessageHeader::Tc::GetSerializedSize() const
{
    ...
    for (const auto& n : advertisedNeighbors)
    {
        size += IPV4_ADDRESS_SIZE; // address
        size += 1;                 // metric count

        for (const auto& m : n.metrics)
        {
            size += 1; // metric type
            size += 2; // metric value
        }
...
MessageHeader::Tc::Print(std::ostream& os) const
{
    ...
    for (const auto& n : advertisedNeighbors)
    {
        ...
        for (const auto& m : n.metrics)
        {
            ...
            os << "type=" << (int)m.type << " value=" << m.value;
        }
...
MessageHeader::Tc::Serialize(Buffer::Iterator start) const
{
    ...
    for (const auto& n : advertisedNeighbors)
    {
        i.WriteHtonU32(n.address.Get());

        uint8_t metricCount = static_cast<uint8_t>(n.metrics.size());;
        i.WriteU8(metricCount);

        for (const auto& m : n.metrics)
        {
            i.WriteU8(m.type);
            i.WriteHtonU16(m.value);
        }
    }
}
...
MessageHeader::Tc::Deserialize(Buffer::Iterator start, uint32_t messageSize)
{
    ...
    this->advertisedNeighbors.clear();
    NS_ASSERT(messageSize >= 4);
    ...
    while (bytesRead < messageSize)
    {
        ...
        n.address = Ipv4Address(i.ReadNtohU32());
        bytesRead += 4;

        uint8_t metricCount = i.ReadU8();
        bytesRead += 1;

        for (uint8_t k = 0; k < metricCount; k++)
        {
            LinkMetric m;

            m.type = i.ReadU8();
            m.value = i.ReadNtohU16();

            n.metrics.push_back(m);

            bytesRead += 3;

            NS_ASSERT(bytesRead <= messageSize);
        }

        advertisedNeighbors.push_back(n);
    }
